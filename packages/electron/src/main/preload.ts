...
contextBridge.exposeInMainWorld("ccr", {
  ...
  exportRequestLogs: (request: RequestLogExportRequest) => invoke(IPC_CHANNELS.appExportRequestLogs, request) as Promise<RequestLogExportResult>,
  exportTextFile: (request: AppExportTextFileRequest) => invoke(IPC_CHANNELS.appExportTextFile, request) as Promise<AppExportTextFileResult>,
